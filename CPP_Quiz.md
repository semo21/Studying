...
- 생성자와 멤버 함수의 차이점을 두 가지 이상 기술하시오.
  ...
  > <span style="color: #44B444">생성자와 소멸자는 이름이 정해져있다. 생성자의 이름은 클래스명과 일치하며, 소멸자의 이름은 클래스명 앞에 ~를 붙인다.</span>
  >
  > <span style="color: #44B444">생성자와 소멸자의 이름이 정해져 있단 것 외에도 멤버 함수와 다른 점은 반환 타입이 없다는 것이다. 반환 값이 void인 것과는 다른 개념으로 반환 값이 존재하지 않는다. 반환 타입 자체를 기술하지 않는다.</span>
  >
  > <span style="color: #44B444">마지막으로 생서자는 인자 타입열을 다르게 해서 중복 정의할 수 있다. 하지만 소멸자는 오직 단 하나만 정의할 수 있고 인자가 없다.</span>

- 생성자는 중복 정의를 할 수 있으나 소멸자는 오직 단 하나만 존재할 수 있는데, 그 이유를 기술하시오.

  > <span style="color: #44B444">클래스 객체는 다양한 멤버 데이터 값을 가지면서 여러 가지 상태를 가질 수 있으나 객체가 소멸한 이후는 아무 것도 남지 않는 유일한 상태이므로 굳이 소멸자를 중복 정의를 통해 여러 개를 가질 필요가 없다.</span>

- 암시적 생성자가 컴파일러에 의해서 논리적으로 추가되는 경우를 기술하시오.

  > <span style="color: #44B444">일반적으로 클래스에 명시적 생성자가 단 하나도 존재하지 않을 경우 컴파일러는 암시적 기본 생성자와 암시적 소멸자를 정의한다.</span>

- 상속 관계 클래스에서 소멸자의 호출 순서는 <span style="color: #44B444">자식</span> 클래스 소멸자, <span style="color: #44B444">부모</span> 클래스 소멸자이다.
